...
Program         :       CLASS ID LBRACE Declarations RBRACE                 {$$ = raiz = newNode("Program"); sprintf(message,"Id(%s)",$2); $$->child = newNode(strdup(message)); addBrother($$->child , $4); }
                ;
...
FieldDecl       :       PUBLIC STATIC Type ID Variaveis SEMICOLON           {$$ = newNode("FieldDecl"); $$->child = $3; sprintf(message,"Id(%s)",$4);struct node * id = newNode(strdup(message)); addBrother($$->child,id); addBrother($$,$5);}
                |       error SEMICOLON                                     {$$ = newNode(NULL);erroSintax = 1;}
                ;

Variaveis       :       COMMA ID Variaveis                                  {$$ = newNode("FieldDecl"); struct node * type = newNode(strdup(lastType));$$->child = type; sprintf(message,"Id(%s)",$2); struct node * id = newNode(strdup(message)); addBrother($$->child,id); addBrother($$,$3); }
                |                                                           {$$ = NULL;}
                ;
...
MethodInvocation:       ID LPAR Expr CommaExpr RPAR                         {$$ = newNode("Call"); sprintf(message, "Id(%s)", $1); $$->child = newNode(strdup(message)); addBrother($$->child, $3); addBrother($3, $4);}
                |       ID LPAR RPAR                                        {$$ = newNode("Call"); sprintf(message, "Id(%s)", $1); $$->child = newNode(strdup(message));}
                |       ID LPAR error RPAR                                  {$$ = newNode(NULL);erroSintax = 1;}
                ;
...
Assignment      :       ID ASSIGN Expr                                      {$$ = newNode("Assign"); sprintf(message, "Id(%s)", $1); $$->child = newNode(strdup(message)); addBrother($$->child, $3);}
                ;

ParseArgs       :       PARSEINT LPAR ID LSQ Expr RSQ RPAR                  {$$ = newNode("ParseArgs"); sprintf(message, "Id(%s)", $3); $$->child = newNode(strdup(message)); addBrother($$->child, $5);}
                |       PARSEINT LPAR error RPAR                            {$$=newNode(NULL);erroSintax = 1;}    
                ;
...
Expr2           :       Expr2 PLUS Expr2                                    {$$ = newNode("Add"); $$->child=$1; addBrother($1,$3);}
                ...
                |       MethodInvocation                                    {$$ = $1;}
                |       ParseArgs                                           {$$ = $1;}
                |       ID DOTLENGTH                                        {$$ = newNode("Length"); sprintf(message, "Id(%s)", $1); $$->child = newNode(strdup(message));} 
                ...
                |       LPAR error RPAR                                     {$$=newNode(NULL); erroSintax = 1;}
                ;
